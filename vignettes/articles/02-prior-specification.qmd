...
```{r}
res_uninformative <- uninformative_analysis@results_summ|>
  select(
    n_total,
    median,
    sd,
...
```{r}
res_informative <- informative_analysis@results_summ|>
  select(
    n_total,
    median,
    sd,
...
res_alpha <- 
  rbind(
    alpha_uninformative@results_summ |>
      select(
        n_total,
        pr_eff,
        pwr_eff
      ...
      dplyr::mutate(Prior = "uninformative", .before = everything()),
    alpha_informative@results_summ |>
      select(
        n_total,
        pr_eff,
        pwr_eff
