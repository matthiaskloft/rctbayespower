...
#' @importFrom purrr pmap map in_parallel
...
#' )
#' }
#' Self-contained function to process a single combination
#'
#' This function processes a single sample size/effect size combination.
#' It's designed to be self-contained for parallel processing with purrr.
#'
#' @param sample_size Total sample size for this combination
#' @param effect_size Effect size for this combination
#' @param combination_id Unique identifier for this combination
#' @param ... All other parameters needed for power analysis (passed from main function)
#' @keywords internal
process_single_combination <- function(sample_size,
                                       effect_size,
                                       combination_id,
                                       percent_group_treat,
                                       n_simulations,
  ...
  n_treatment <- round(sample_size * percent_group_treat)
  n_control <- sample_size - n_treatment

  # Get cached models for this effect size
  compiled_models <- compiled_models_cache[[as.character(effect_size)]]

  # Build power analysis arguments
  power_args <- c(
    list(
      n_control = n_control,
  ...
  # Add cached models if available
  ...
  result <- tryCatch({
    ...
    cat("ERROR in combination", combination_id, 
        "(N =", sample_size, ", Effect =", effect_size, "):\n")
    cat("  ", error_msg, "\n")
    
    # If it's a brms compilation error, suggest faster algorithm
    if (grepl("Stan|compilation|sampling", error_msg, ignore.case = TRUE)) {
      cat("  SUGGESTION: Try adding brms_args = list(algorithm = 'meanfield') for faster compilation\n")
    }
...
power_grid_analysis <- function(target_power_success = 0.9,
  ...
  # Validate required parameters for ANCOVA to prevent common NA-causing errors
  if (power_analysis_fn == "power_analysis_ancova") {
    required_ancova_args <- c("outcome_type", "baseline_effect")
    missing_args <- setdiff(required_ancova_args, names(list(...)))
    if (length(missing_args) > 0) {
      stop("For power_analysis_ancova, missing required arguments: ", 
           paste(missing_args, collapse = ", "),
           "\n  Required: outcome_type, baseline_effect")
    }
  }
  ...
  # Add default brms_args if not provided to help avoid compilation issues
  if (is.null(dots$brms_args)) {
    dots$brms_args <- list(
      algorithm = "meanfield",
    ...
    cat("NOTE: No brms_args provided. Using fast defaults:\n")
    cat("  algorithm = 'meanfield', iter = 50000, output_samples = 2000\n")
    cat("  For production use, consider sampling algorithms for better accuracy.\n\n")
  }
    ...
    if (power_analysis_fn == "power_analysis") {
      ...
      tryCatch({
        ...
        if (all(required_params %in% names(dots))) {
          ...
          compilation_error <- paste(
            "Missing required parameters for power_analysis:",
      ...
      tryCatch({
        ...
        if (all(required_ancova_params %in% names(dots))) {
          ...
          compilation_error <- paste(
            "Missing required parameters for power_analysis_ancova:",
            paste(missing_params, collapse = ", ")
          )
        }
      }, error = function(e) {
  ...
  if (n_cores > 1 && length(unique_effects) > 1) {
    ...
    cl <- tryCatch({
      ...
      warning(
        "Failed to create parallel cluster: ",
        e$message,
        ". Falling back to sequential compilation."
      )
      return(NULL)
    })
  ...
  for (i in seq_along(compilation_results)) {
    ...
    if (is.null(result$error)) {
      ...
      cat("✓ Successfully compiled models for effect size",
          effect_size,
          "\n")
    } else {
      compiled_models_cache[[as.character(effect_size)]] <- NULL
      cat("✗ Failed to compile models for effect size",
          effect_size,
          ":",
          result$error,
          "\n")
  ...
  cat("\n=== Phase 2: Grid-Level Parallel Analysis ===\n")

  # Process all combinations with purrr 1.1.0 parallel grid processing
  n_total_combinations <- nrow(combinations)
  cat("Processing", n_total_combinations, "combinations")

  # Prepare parameters for the process function
  process_params <- list(
    ...
    compiled_models_cache = compiled_models_cache,
    dots = dots
  )

  # Check if purrr >= 1.1.0 and mirai are available for in_parallel()
  purrr_version <- packageVersion("purrr")
  has_in_parallel <- purrr_version >= "1.1.0" &&
    requireNamespace("mirai", quietly = TRUE)

  if (n_cores > 1 && n_total_combinations > 1) {
    if (has_in_parallel) {
      cat(" in parallel using",
          n_cores,
          "cores with purrr in_parallel()...\n")

      # Set up mirai daemons for parallel processing
      if (requireNamespace("mirai", quietly = TRUE)) {
        mirai::daemons(n_cores)
        
        # Export required functions to mirai workers
        tryCatch({
          mirai::export(list(
            process_single_combination = process_single_combination,
            power_analysis = power_analysis,
            power_analysis_ancova = power_analysis_ancova,
            validate_power_design = validate_power_design
          ))
          cat("  Functions exported to mirai workers successfully\n")
        }, error = function(e) {
          warning("Failed to export functions to mirai workers: ", e$message)
        })
        
        # Import required packages on workers
        tryCatch({
          mirai::import("brms")
          mirai::import("rctbayespower")
          mirai::import("dplyr")
          mirai::import("stats")
          cat("  Packages imported on mirai workers successfully\n")
        }, error = function(e) {
          warning("Failed to import packages on mirai workers: ", e$message)
        })
        
      } else {
        stop("mirai package is required for purrr in_parallel() but not available")
      }

      # Process combinations in parallel using purrr 1.1.0
      results_list <- tryCatch({
        combinations %>%
          dplyr::mutate(combination_id = seq_len(nrow(.))) %>%
          purrr::pmap(
            ~ process_single_combination(
              sample_size = ..1,
              effect_size = ..2,
              combination_id = ..3,
              percent_group_treat = process_params$percent_group_treat,
              n_simulations = process_params$n_simulations,
              threshold_success = process_params$threshold_success,
              threshold_futility = process_params$threshold_futility,
              power_analysis_fn = process_params$power_analysis_fn,
              compiled_models_cache = process_params$compiled_models_cache,
              dots = process_params$dots
            ),
            .options = purrr::in_parallel(.progress = TRUE)
          )
      }, finally = {
      ...
      # Fallback to parallel package for older purrr versions
      cat(" in parallel using",
          n_cores,
          "cores with parallel package (purrr < 1.1.0)...\n")
      ...
      results_list <- tryCatch({
        ...
        combinations_with_id <- dplyr::mutate(combinations, combination_id = seq_len(nrow(combinations)))

        parallel::parLapply(cl, seq_len(nrow(combinations_with_id)), function(i) {
          row <- combinations_with_id[i, ]
          process_single_combination(
    ...
    # Sequential processing using purrr
  ...
  # Convert results list to matrix format for compatibility
  results_matrix <- results_list

  elapsed_time <- difftime(Sys.time(), start_time, units = "mins")
  cat("\nTotal analysis time:", round(as.numeric(elapsed_time), 2), "minutes\n")
  
  # Check for and report failed combinations
  failed_combinations <- results_list[sapply(results_list, function(x) !x$success)]
  if (length(failed_combinations) > 0) {
    cat("\nWARNING:", length(failed_combinations), "out of", length(results_list), 
        "combinations failed and returned NA values\n")
    cat("Check the error messages above for details on why these combinations failed.\n")
  }

  # Create power surface data frame with simplified logic
  power_surface <- do.call(rbind, lapply(results_matrix, function(x) {
    data.frame(
      n_total = if (is.null(x$n_total))
        NA_integer_
      else
        x$n_total,
      n_control = if (is.null(x$n_control))
        NA_integer_
      else
        x$n_control,
      n_treatment = if (is.null(x$n_treatment))
        NA_integer_
      else
        x$n_treatment,
      effect_size = if (is.null(x$effect_size))
        NA_real_
      else
        x$effect_size,
  ...
  if (!is.null(weight_fn)) {
    ...
    for (n in sample_sizes) {
      ...
      if (nrow(subset_data) > 0 &&
        ...
        mcse_integrated_power_success <- calculate_mcse_integrated_power(subset_data$power_success,
                                                                         weights,
                                                                         n_simulations,
                                                                         is_power_metric = TRUE)
        mcse_integrated_power_futility <- calculate_mcse_integrated_power(subset_data$power_futility,
                                                                          weights,
                                                                          n_simulations,
                                                                          is_power_metric = TRUE)
        mcse_integrated_prob_success <- calculate_mcse_integrated_power(subset_data$mean_prob_success,
                                                                        weights,
                                                                        n_simulations,
                                                                        is_power_metric = FALSE)
        mcse_integrated_prob_futility <- calculate_mcse_integrated_power(subset_data$mean_prob_futility,
                                                                         weights,
                                                                         n_simulations,
                                                                         is_power_metric = FALSE)
...
print.rctbayespower_grid <- function(x, ...) {
  ...
  if (x$analysis_type == "effect_only") {
    ...
    cat("Minimum required total sample sizes: Success =", if (!is.na(x$min_n_success)) {
      x$min_n_success
    } else {
      "Not achieved"
    })
...
summary.rctbayespower_grid <- function(object,
  ...
  if (!is.null(design_prior)) {
    ...
    if (is.null(n_sims_for_mcse) ||
        !is.numeric(n_sims_for_mcse) || n_sims_for_mcse <= 0) {
      n_sims_for_mcse <- 1000  # Safe default
      warning(
        "n_simulations not found or invalid in result object, using default value of 1000 for MCSE calculations"
      )
    }
  ...
  if (print) {
    ...
    if (!is.null(object$optimal_combinations_success) &&
      ...
      names(success_display) <- c("N_total",
                                  "Effect_size",
                                  "Power_success",
                                  "Prob_success")
      if (print) {
    ...
    if (!is.null(integrated_power_to_display)) {
      ...
      if (!is.null(runtime_integrated_power)) {
        ...
        cat("(Using design prior from main analysis:",
            object$design_prior,
            ")\n")
      }
      cat("\n")
      ...
      if ("mcse_integrated_power_success" %in% names(integrated_display)) {
        integrated_display$integrated_power_success <- paste0(
          ...
          "% (±",
          round(
            integrated_display$mcse_integrated_power_success * 100,
            2
          ),
          ")"
        )
        integrated_display$integrated_power_futility <- paste0(
          ...
          "% (±",
          round(
            integrated_display$mcse_integrated_power_futility * 100,
            2
          ),
          ")"
        )
        integrated_display$integrated_prob_success <- paste0(
          ...
          "% (±",
          round(
            integrated_display$mcse_integrated_prob_success * 100,
            2
          ),
          ")"
        )
        integrated_display$integrated_prob_futility <- paste0(
          ...
          "% (±",
          round(
            integrated_display$mcse_integrated_prob_futility * 100,
            2
          ),
          ")"
        )
      ...
      cat("Values shown as percentage (±MCSE) where MCSE = Monte Carlo Standard Error.\n")
    }
